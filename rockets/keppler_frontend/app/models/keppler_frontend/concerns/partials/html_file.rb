      ...
      module HtmlFile
        ...
        def html_code
          ...
          index_html.each do |idx|
            begin_idx = index_html.find_index(idx) if idx.include?("<partial id='#{name}'>\n")
            end_idx = index_html.find_index(idx) if idx.include?("</partial>\n")
          end
        ...
        def install_html
          ...
          index_html = File.readlines(file)
          head_idx = 0
          index_html.insert(head_idx.to_i + 1, "<partial id='#{name}'>\n")
        ...
        def update_html(html)
          ...
          index_html[begin_idx] = "<partial id='#{html[:name]}'>\n"
          index_html[end_idx] = "</partial>"
        ...
        def save_html(code)
          ...
          end_idx = 0
          code_html.each do |i|
            begin_idx = code_html.find_index(i) if i.include?("<partial id='#{name}'>\n")
            end_idx = code_html.find_index(i) if i.include?("</partial>\n")
          end
          # return if begin_idx==0
