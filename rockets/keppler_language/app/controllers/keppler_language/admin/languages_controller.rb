require_dependency "keppler_language/application_controller"
module KepplerLanguage
  module Admin
    # LanguagesController
    class LanguagesController < ApplicationController
      layout 'keppler_language/admin/layouts/application'
      before_action :set_language, only: [:show, :edit, :update, :destroy]
      before_action :show_history, only: [:index]
      before_action :set_attachments
      before_action :authorization
      include KepplerLanguage::Concerns::Commons
      include KepplerLanguage::Concerns::History
      include KepplerLanguage::Concerns::DestroyMultiple
      ...
      def clone
        ...
        if @language.save
          redirect_to admin_language_languages_path
        else
          render :new
        end
      end
      ...
      def destroy
        @language.destroy
        redirect_to admin_language_languages_path, notice: actions_messages(@language)
      end
      ...
      def redirect(object, commit)
        if commit.key?('_save')
          redirect_to([:admin, :language, object], notice: actions_messages(object))
        elsif commit.key?('_add_other')
          redirect_to(
            send("new_admin_language_#{underscore(object)}_path"),
